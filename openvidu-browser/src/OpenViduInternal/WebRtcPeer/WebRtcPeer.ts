...
export class WebRtcPeer {
	...
	constructor(protected configuration: WebRtcPeerConfiguration) {
		...
		this.pc.onicecandidate = (event) => {
			if (!!event.candidate) {
				...
				if (candidate) {
					this.localCandidatesQueue.push(<RTCIceCandidate>{
	...
	generateOffer(): Promise<string> {
		return new Promise((resolve, reject) => {
			...
			if (!!this.configuration.mediaConstraints) {
				offerAudio =
					typeof this.configuration.mediaConstraints.audio ===
					"boolean"
						? this.configuration.mediaConstraints.audio
						: true;
				offerVideo =
					typeof this.configuration.mediaConstraints.video ===
					"boolean"
			...
			const constraints: RTCOfferOptions = {
				offerToReceiveAudio:
					this.configuration.mode !== "sendonly" && offerAudio,
				offerToReceiveVideo:
					this.configuration.mode !== "sendonly" && offerVideo,
			};
				...
				this.pc
					...
					.then((offer) => {
						...
						offer.sdp = offer.sdp?.replace(
							...
							"stereo=1;sprop-stereo=1;maxaveragebitrate=510000;usedtx=1;cbr=1;useinbandfec=1;ptime=20;minptime=20;maxptime=20;maxplaybackrate=48000;sprop-maxcapturerate=48000;x-google-min-bitrate=128;x-google-max-bitrate=144"
						);
						return this.pc.setLocalDescription(offer);
					})
					.then(() => {
	...
	processAnswer(
		sdpAnswer: string,
		needsTimeoutOnProcessAnswer: boolean
	): Promise<string> {
		return new Promise((resolve, reject) => {
	...
	addIceConnectionStateChangeListener(otherId: string) {
		this.pc.oniceconnectionstatechange = () => {
			const iceConnectionState: RTCIceConnectionState = this.pc
				.iceConnectionState;
